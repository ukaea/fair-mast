...
class ShotModel(SQLModel, table=True):
    ...
    cpf_p03249: Optional[float] = Field(alias="mcs_gdc_ps",
                                        description="(MCS Setup) GDC completed prior to shot for MAST only")

    cpf_p04673: Optional[float] = Field(alias="mcs_select_pv1",
                                        description="(MCS Setup) select gas group 1 for MAST only")

    cpf_p04674: Optional[float] = Field(alias="mcs_select_pv2",
                                        description="(MCS Setup) select gas group 2 for MAST only")

    cpf_p04675: Optional[float] = Field(alias="mcs_select_pv3",
                                        description="(MCS Setup) select gas group 3 for MAST only")

    cpf_p04676: Optional[float] = Field(alias="mcs_select_pv4",
                                        description="(MCS Setup) select gas group 4 for MAST only")

    cpf_p04677: Optional[float] = Field(alias="mcs_select_pv5",
                                        description="(MCS Setup) select gas group 5 for MAST only")

    cpf_p04678: Optional[float] = Field(alias="mcs_select_pv6",
                                        description="(MCS Setup) select gas group 6 for MAST only")

    cpf_p04679: Optional[float] = Field(alias="mcs_select_pv7",
                                        description="(MCS Setup) select gas group 7 for MAST only")

    cpf_p04680: Optional[float] = Field(alias="mcs_select_pv8",
                                        description="(MCS Setup) select gas group 8 for MAST only")

    cpf_p04681: Optional[float] = Field(alias="mcs_select_pv9",
                                        description="(MCS Setup) select gas group 9 for MAST only")

    cpf_p04833: Optional[float] = Field(alias="mcs_select_lvps",
                                        description="(MCS Setup) select TF power supply for MAST only")

    cpf_p04993: Optional[float] = Field(alias="mcs_select_cp3s",
    ...
    cpf_p05027: Optional[float] = Field(alias="mcs_select_cp3c",
                                        description="(MCS Setup) select P3 counterpulse bank for MAST only")

    cpf_p05028: Optional[float] = Field(alias="mcs_select_cp4",
                                        description="(MCS Setup) select P4 start bank for MAST only")

    cpf_p05029: Optional[float] = Field(alias="mcs_select_cp5",
                                        description="(MCS Setup) select P3 start bank for MAST only")

    cpf_p05030: Optional[float] = Field(alias="mcs_select_p3p_s",
                                        description="(MCS Setup) select Siemens & GEC switches for P3 plasma Initiation for MAST only")

    cpf_p05032: Optional[float] = Field(alias="mcs_select_p4_thy",
                                        description="(MCS Setup) select P4 Ignitron and Thyristor circuit for MAST only")

    cpf_p05033: Optional[float] = Field(alias="mcs_select_p5_thy",
    ...
    cpf_p10963: Optional[float] = Field(alias="mcs_select_ss_nbi_gas",
                                        description="(MCS Setup) MCS_SS_NBI_gas_species for MAST only")

    cpf_p10964: Optional[float] = Field(alias="mcs_select_sw_nbi_gas_mcs",
                                        description="(MCS Setup) MCS_SW_NBI_gas_species for MAST only")

    cpf_p12441: Optional[float] = Field(alias="mcs_cp3s_volts",
                                        description="(MCS Setup) CP3 set volts for MAST only")

    cpf_p12450: Optional[float] = Field(alias="mcs_cp3c_volts")

    cpf_p12451: Optional[float] = Field(alias="mcs_cp4s_volts",
    ...
    cpf_p21076: Optional[float] = Field(alias="mcs_efps_duration",
                                        description="(MCS Setup) EFPS enable duration for MAST only")

    cpf_p21077: Optional[float] = Field(alias="mcs_sfps_duration",
                                        description="(MCS Setup) SFPS enable duration for MAST only")

    cpf_p21078: Optional[float] = Field(alias="mcs_p1ps_pos_current_limit",
    ...
    cpf_exp_date: Optional[datetime.datetime] = Field(alias="shot_exp_date",
                                                      description="MAST shot experiment pulse date for MAST and MAST-U")

    cpf_exp_number: Optional[int] = Field(alias="shot_exp_number",
    ...
    cpf_o2ratio: Optional[float] = Field(alias="rad_o2ratio",
    ...
    cpf_pe0_max: Optional[float] = Field(alias="thom_e_pressure_max",
    ...
    cpf_preshot: Optional[str] = Field(alias = "shot_preshot",
    ...
    cpf_te0_max: Optional[float] = Field(alias="thom_e_temp_max",
    ...
    cpf_tend_nbi_ss: Optional[float] = Field(alias="nbi_end_time_ss",description="SS NBI end time for MAST and MAST-U")
