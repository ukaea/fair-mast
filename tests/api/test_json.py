...
def test_get_cpf(client, override_get_db):
    response = client.get("/json/dataset/cpf_summary")
...
def test_get_shots(client, override_get_db):
    response = client.get("json/dataset/shots")
    data = response.json()
    assert response.status_code == 200
    assert len(data["items"]) == 50
...
def test_get_shots_filter_shot_id(client, override_get_db):
    response = client.get("json/dataset/shots?filters=shot_id$geq:30000")
    data = response.json()
    assert response.status_code == 200
    assert len(data["items"]) == 50
...
def test_get_shot(client, override_get_db):
    response = client.get("json/dataset/shots/30420")
    data = response.json()
    assert response.status_code == 200
...
def test_get_shot_aggregate(client, override_get_db):
    response = client.get(
        "json/dataset/shots/aggregate?data=shot_id$min:,shot_id$max:&groupby=campaign&sort=-min_shot_id"
    )
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 1
...
def test_get_signals_aggregate(client, override_get_db):
    response = client.get("json/dataset/signals/aggregate?data=shot_id$count:&groupby=quality")
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 2
...
def test_get_signals_for_shot(client, override_get_db):
    response = client.get("json/dataset/shots/30471/signals")
    data = response.json()
    assert response.status_code == 200
    assert len(data["items"]) == 50
    assert data["previous_page"] is None
...
def test_get_signals(client, override_get_db):
    response = client.get("json/dataset/signals")
    data = response.json()
    assert response.status_code == 200
...
def test_get_scenarios(client, override_get_db):
    response = client.get("json/dataset/scenarios")
    data = response.json()
    assert response.status_code == 200
...
def test_get_sources(client, override_get_db):
    response = client.get("json/dataset/sources")
    data = response.json()
    assert response.status_code == 200
    assert len(data["items"]) == 50
...
def test_get_cursor(client, override_get_db):
    ...
    next_response = client.get(f"json/dataset/signals?cursor={next_cursor}")
