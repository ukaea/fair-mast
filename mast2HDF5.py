...
def write_file(shot):
    process = current_process()._identity[0] - 1
    client = pyuda.Client()
    ...
    with h5py.File(file_path, 'a') as file:
        ...
        for entry in tqdm(
                cpf_categories,
                desc=f"Converting cpf for shot {shot}",
                position=process,
                leave=False,
        ):
        ...
        for source, signal_list in tqdm(
                source_dict.items(),
                desc=f"Converting signal data for shot {shot}",
                position=process * 2,
                leave=False,
        ):
            for signal_name in tqdm(
                ...
                if type(data) == pyuda._signal.Signal:
                    ...
                    errors = group.create_dataset("errors", data=data.errors)
                    if data.time:
                        time = group.create_dataset(
                                "time", data=data.time.data
                        )
                        time.attrs["units"] = data.time.units

        images = file.create_group("images")
        for image_source in tqdm(
                image_sources,
                desc=f"Converting image data for shot {shot}.",
                position=process,
        ):
            ...
            frame_times = image_group.create_dataset(
                    "frame_times",
            ...
            for frame in image_data.frames:
                frame_group = frames_group.create_group(
                        f"frame_{frame.number}"
                )
                keys = dir(frame)
                keys = [key for key in keys if not key.startswith('_')]
                for key in keys:
                    frame_data = frame_group.create_dataset(
                            key, data=getattr(frame, key)
                    )
...
if __name__ == "__main__":
    ...
    with Pool(processes) as pool:
        pool.map(
            write_file,
            random.sample(shot_list, processes) 
        )

    execution_time = time.time() - start_time
    with open("times.txt", 'a') as file:
        file.write(
            f"{processes},{datetime.timedelta(seconds=execution_time)}\n"
        )
